...
impl<const N: usize> EncryptedData<N> {
    ...
    pub fn decrypt(mut self, key: &[u8]) -> Result<FixedOrDynamicArray<N>> {
        ...
        decrypt.decrypt_in_place_detached(&GenericArray::from(self.nonce), &[], self.data.as_mut_slice(), &GenericArray::from(self.mac)).map_err(|err| DecryptionError(err))?;
    ...
    pub fn encrypt(key: &[u8], mut data: FixedOrDynamicArray<N>) -> Result<EncryptedData<N>> {
        ...
        let mac = encrypt.encrypt_in_place_detached(&nonce, &[], data.as_mut_slice()).map_err(|err| EncryptionError(err))?;
