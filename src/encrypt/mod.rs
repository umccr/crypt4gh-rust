...
use bytes::Bytes;
use crate::encrypt::header::Header;
use crate::Keys;
  ...
  pub fn poll_session_keys(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<()>> {
    ...
    match ready!(this.inner.poll_next(cx)) {
      Some(Ok(buf)) => match buf {
        ...
        DecodedBlock::DataBlock(_) => Poll::Ready(Err(Crypt4GHError::NoKey(
          "data block reached without finding session keys".to_string(),
        ))),
      },
      Some(Err(e)) => Poll::Ready(Err(e)),
      None => Poll::Ready(Err(Crypt4GHError::NoKey(
  ...
  R: AsyncRead + AsyncSeek + Unpin + Send,
{
  ...
  pub async fn seek_encrypted(&mut self, position: SeekFrom) -> Result<u64, Crypt4GHError> {
  ...
  pub async fn seek_unencrypted(&mut self, position: u64) -> io::Result<u64> {
    ...
    let position = self
      ...
      .ok_or_else(|| Crypt4GHError::InvalidPosition("Unable to convert to encrypted position.".to_string()))?;

    // Then do the seek.
    self.seek_encrypted(SeekFrom::Start(position)).map_err(Crypt4GHError::IoError(()))
  }
}
